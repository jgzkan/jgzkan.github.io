...
- 앙상블 학습(ensemble learning): 대부분 결정 트리를 기반으로 만들어진, 정형 데이터를 다루는데 가장 뛰어난 성과를 내는 알고리즘

### 랜덤 포레스트 ###

> 랜덤 포레스트(random forest): 안정적인 성능 덕분에 널리 사용되고 있는 앙상플 학습의 대표 주자다. 이름에서 유추할 수 있듯이, 결정 트리를 랜덤하게 만들어 결정 트리의 숲을 만든다. 그리고 각 결정 트리의 예측을 사용해 최종 예측을 만든다. 
