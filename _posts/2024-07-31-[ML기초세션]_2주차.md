...
supervised: 입력과 타깃으로 이뤄진 훈련 데이터가 필요하다. 
unsupervised: 타깃 없이 입력 데이터만 사용, 무엇을 예측하는게 아니라 어떤 특징을 찾는데 주로 활용한다. 

>supervised learning은 labeled observation, 즉 이미 라벨링된, 정답이 있는 데이터들을 가진다. 
unsupervised learning은 데이터에서 숨겨진 패턴을 찾는 것에 목적을 둔다.
...
supervised learning과 unsupervised learning의 가장 큰 차이점은 학습 시 정답(Label)의 제공 여부이다. 


- 데이터가 클수록 파이썬의 리스트는 비효율적이므로 numpy의 array를 쓰는게 좋다 --> why?
